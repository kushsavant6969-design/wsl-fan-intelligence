...
from data import get_full_club_data, WSL_CLUBS, WSL_LEAGUE_CONTEXT
...
def kpi_html(label, value, delta, color="#c8f135", delta_color="#22c55e"):
    return f"""
...
with k1: st.markdown(kpi_html("Sentiment Score", kpis["sentiment_score"],
    f"{'▲' if sent_dir=='up' else '▼'} {kpis['sentiment_score']-65:+d} vs league avg",
    "#c8f135", "#22c55e" if sent_dir=="up" else "#ef4444"), unsafe_allow_html=True)
...
st.markdown("<hr style='border-color:#1f2937;margin:8px 0 16px'>", unsafe_allow_html=True)
...
lc = st.columns(len(WSL_LEAGUE_CONTEXT))
for i, (club, data) in enumerate(WSL_LEAGUE_CONTEXT.items()):
